...
# ----------------------------
# Tekenfunctie Hoek (B = gezamenlijke diepte)
# ----------------------------

def draw_hoek(A, B, D, strip, X=None):
    ...
    # Kort been (zelfde diepte B)
    ax.add_patch(
        Rectangle((0, B), D, B, facecolor="white", edgecolor="black", linewidth=1.5)
    )
    ...
    # Strip kort been
    ax.add_patch(
        Rectangle((0, B), strip, B, facecolor="#d9d9d9", edgecolor="black")
    )
    ...
    ax.text(D / 2, B * 2 + 6, "D", ha="center", fontsize=10)
    ...
    # ---- View limits ----
    ax.set_xlim(-30, max(A, D) + 30)
    ax.set_ylim(-30, B * 2 + 30)
...
with st.sidebar:
    ...
    B = st.number_input("B – diepte (beide benen)", 40.0, 400.0, 100.0, step=5.0)
...
fig = draw_hoek(A, B, D, strip, X if X > 0 else None)
st.pyplot(fig)

st.subheader("Samenvatting")
st.write(f"**A** = {A:.0f} mm")
st.write(f"**B** = {B:.0f} mm (beide benen)")
...
st.caption(
    "Schematische hoek-tekening met één gezamenlijke diepte (B), "
    "conform praktijk en formulier."
)
